...
fun NavMenuItem(
    ...
    CompositionLocalProvider(LocalRippleTheme provides rippleTheme) {
        Row(
            ...
            Icon(
                tint = if (selected) AppColors.drawerSelected else AppColors.drawerContent,
                modifier = modifier.padding(end = 10.dp),
                imageVector = icon,
                contentDescription = ""
            )
            Text(title, color = if (selected) AppColors.drawerSelected else AppColors.drawerContent)
        }
    }
}